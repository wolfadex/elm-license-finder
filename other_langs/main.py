...
def build_output_tables(deps):
    ...
    lsc_count = {"total": 0, "direct": 0, "indirect": 0}
    for pkg in packages:
        pkg_data = deps[pkg]
        license = pkg_data["license"]
        if license not in lsc_count.keys():
            lsc_count[license] = 0
        lsc_count[license] += 1
        lsc_count["total"] += 1
        lsc_count[pkg_data["type"]] += 1
        large_table_data.append(
            [pkg, pkg_data["version"], license, pkg_data["type"]]
        )
    for l, c in lsc_count.items():
    ...
    print("Dependencies:")
    print(f"Total: {lsc_data['total']}")
    print(f"Direct: {lsc_data['direct']}")
    print(f"Indirect: {lsc_data['indirect']}")
